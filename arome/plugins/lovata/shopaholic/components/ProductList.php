...
class ProductList extends SortingElementList
{
    ...
    public function StringToInt($str){
        $result = intval(str_replace(' ','',$str));
        return $result;
    }

    public function onSearchProductList(){

        // return Input::all();
        return Redirect::to('./all-product')->with(['data' => Input::all()]);
    }

    public function onRun(){
        if(Session::get('data')){
            $this->page['search'] = Session::get('data')['search-pc'];
            Session::forget('data');
        }
        
    }

    public function filterProductList($data, $search){
        foreach($data as $key => $item){
            if(!stristr($item->name, $search)) unset($data[$key]);
        }
        return $data;
    }
}
