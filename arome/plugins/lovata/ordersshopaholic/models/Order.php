...
class Order extends Model
{
    ...
    protected function getRelationAll(){
        return $this->with([
            'order_position',
            'order_offer',
            'order_promo_mechanism',
    ...
    protected function getCompleteOrder(){
        return $this->where('status_id', $this::completeOrder);
    }

    protected function filterThisMonth($obj){
        return $obj->whereMonth("created_at",Carbon::now()->month);
    }

    protected function filterThisWeek($obj){
        return $obj->whereBetween('created_at', [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()]);
    }

    protected function filterThisYear($obj){
        return $obj->whereYear("created_at",Carbon::now()->year);
    }

    protected function ListOrderThisMonth(){
        $listOrder = $this->getCompleteOrder();
        return $this->filterThisMonth($listOrder);
    }

    protected function ListOrderThisMonthHaveRelation(){
        $listOrder = $this->getCompleteOrder()->with('order_offer.offer');
        return $this->filterThisMonth($listOrder);
    }

    protected function ListOrderThisYear(){
        $listOrder = $this->getCompleteOrder();
        return $this->filterThisYear($listOrder);
    }

    protected function ListOrderThisWeek(){
        $listOrder = $this->getCompleteOrder();
        return $this->filterThisWeek($listOrder);
    }
}